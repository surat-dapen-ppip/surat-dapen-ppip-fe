...
export default function RichEditMemoComponent({
  currentDocument,
  setCurrentDocument,
  triggerSave,
  onSaveComplete,
  ...
  const [isAbleSave, setIsAbleSave] = useState(false); // Track the save process
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !isEditorReady) {
      ...
      const richEditor = create(document.getElementById("richEditMemo"), options);
  ...
  return (
    <div>
      <div ref={richEditRef} id="richEditMemo"></div>
    </div>
  );
}
