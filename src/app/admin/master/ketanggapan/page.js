...
"use client"
import { createPriority, deletePriority, getPriorities, updatePriority } from "@/services/priorities";
import { Form, message, Modal, Popconfirm, Table } from "antd";
import { useEffect, useState } from "react";

export default function pageKetanggapan() {
    ...
    const handleFinish = async () => {
        let data = formPriority.getFieldsValue()
        try {
            if (eventName == "Tambah") {
                await createPriority(data)
            } else {
                await updatePriority(UID, data)
            }
                    ...
                    <div>
                        <button className="bg-gray-200 text-black text-md font-semibold rounded p-2 mr-2"
                            onClick={() => {
                                setEventName("Update")
                                setUID(record.UID)
                                formPriority.setFieldValue("Name", record.Name)
                                setTimeout(() => {
                                    setIsModalOpen(true)
                                }, 200)
                            }}
    ...
    return (
        <main>
            ...
            <div className="p-6 bg-white shadow-sm rounded mt-5">
                <Table dataSource={dataPriority} columns={columns} />
            </div>
            ...
            >
                ...
                >
                    <Form.Item
                        ...
                        name={'Name'}
                        rules={[{ required: true, message: 'Tolong masukkan nama prioritas' }]}
                    >
                        <input
                            type="text"
                            placeholder="Input Nama Prioritas"
