...
export default function pageSuratKeluar() {
    ...
    const fetchCounter = useCallback(async () => {
        const response = await getMessageCounter(1)
        if (response) {
    ...
    const fetchTypeName = async () => {
        const response = await getTypeNameSurat(1);
        if (response) {
    ...
    const fetchTemplateName = async (typeName) => {
        const response = await getTemplateNameSurat(typeName, 1);
        if (response) {
    ...
    const handleCheckEventAvailability = async (eventNumber, eventNumberSub) => {
        ...
        const response = await checkMessageAvailability(eventNumber, eventNumberSub, 1)
        if (response) {
